...
@interface PublicTableViewController ()<UIAlertViewDelegate>

@property (strong, nonatomic) NSArray *publicModelArray;

@property (nonatomic, strong) PublicCellViewModel *cellViewModel;

@property (nonatomic, strong) PublicCell *cell;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    __weak typeof(PublicCell *) wCell = cell;
    cell.changeColorBlock = ^(){
        weakSelf.cell = wCell;
        weakSelf.cellViewModel = [[PublicCellViewModel alloc] init];
        weakSelf.cellViewModel.model = weakSelf.publicModelArray[indexPath.row];
        UIAlertView *alerView = [[UIAlertView alloc] initWithTitle:nil message:@"改变颜色" delegate:self cancelButtonTitle:@"取消" otherButtonTitles:@"确定", nil];
        [alerView show];
    };
...
{
    if (buttonIndex == 1) {
        [self.cell changeColor];
        [self.cellViewModel sendChangeColorComplete:^{
            NSLog(@"数据提交完成");
        }];
    }
}
